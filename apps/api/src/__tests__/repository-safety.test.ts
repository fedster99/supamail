...
describe("repository safety", () => {
  ...
  it("drops discovery-flagged missing folders from the sync + history working set", async () => {
    ...
    // account. All seven readers must carry the exclusion: getFoldersDueForSync
    // (priority + round-robin), getIdleWatchAccounts, getInboxFolderForWake,
    ...
    expect(collapsed.split(syncFilter).length - 1).toBe(7);
