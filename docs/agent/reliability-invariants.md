...
- Current default sync edges:
  ...
  - A host may use `openInboxIdleSession` for earlier mailbox sync. Inbox `exists`, `expunge`, and `flags` events plus bounded STATUS rotations over tracked non-Inbox folders are only wake hints. The host must reuse that one provider session for sync, keep the periodic full-sync timer, own reconnect/backoff, and use the existing account-locked sync path. `EXPUNGE` must force Inbox reconcile; `FLAGS` must force an Inbox flag scan. A natural IDLE completion yields immediately so the host can take the shared connection for periodic sync.
