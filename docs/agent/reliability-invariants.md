...
- Once `startSyncRun` succeeds, every in-process escape path must make a best-effort terminal `finishSyncRun` write before rethrowing. Account-lock checkout failures, stale-lock recovery failures, and finalization failures must not silently leave a healthy account with a phantom `running` row.
