...
- The 32 KiB UTF-8 `search_extract`, `threading_payload_sha256` over every parsed body variant, and the remaining threading evidence commit to Postgres before full payload storage. Envelope corrections must rebuild delivery projections from this compact evidence, never from stored body columns. Full MIME/parsed payload goes through `BodyStore`; `DatabaseBodyStore` remains the OSS default and message list rows stay metadata-oriented.
