...
| Zoho Mail | Connectivity preset; live smoke pending | `provider-profiles.test.ts` covers the `zoho` preset coordinates + domain autodiscovery. | `zoho` preset (IMAP `imap.zoho.com:993` / SMTP `smtp.zoho.com:465`) autodiscovered from `zoho.com`/`zohomail.com`; sync behavior is generic-core (ADR 0021). |
...
| Yahoo/AOL Mail | Connectivity preset; live smoke pending | `provider-profiles.test.ts` covers the `yahoo` preset coordinates + domain autodiscovery. | `yahoo` preset (IMAP `imap.mail.yahoo.com:993` / SMTP `smtp.mail.yahoo.com:465`) autodiscovered from `yahoo.com`/`ymail.com`/`rocketmail.com`; requires an app password (ADR 0021). Verify throttling and transient disconnect behavior. |
