...
1. **Conformance audit** found three drifts where load-bearing safeguards were defined in config or implied by the reliability contract but never enforced in code: `MAX_LOCK_HOLD_MS`, an initial-sync stall timeout, and the "stuck DEGRADED for 24h → BROKEN" transition. The first two became more urgent once we committed to historical backfill, which holds locks longer and fetches in big batches.
...
- `docs/spec-conformance.md` — public reliability contract and verification matrix
