...
Mechanical: add `INITIAL_SYNC_BATCH_TIMEOUT_MS` to config (default `5 * 60_000`). Wrap `runInitialSyncBatch` (the snapshot SEARCH + ≤1 batch FETCH) in the existing `withOperationDeadline` helper. On timeout: abort IMAP via `client.close()`, throw, treat as transient failure in the outer `syncFolder` catch, do NOT advance `initial_sync_oldest_uid_synced` watermark. Identical handling to `INCREMENTAL_TOTAL_TIMEOUT_MS`.
...
- **`apps/api/src/config.ts`** — add `INITIAL_SYNC_BATCH_TIMEOUT_MS`, `STUCK_DEGRADED_BROKEN_THRESHOLD_MS` (24h), `STUCK_DEGRADED_TERMINAL_THRESHOLD_MS` (7d), `STUCK_DEGRADED_RETRY_INTERVAL_MS` (1h), `MAX_BODY_BATCHES_PER_TICK` (default 4), `FOLDER_COUNT_WARN_THRESHOLD` (50), `FOLDER_COUNT_ENFORCE_THRESHOLD` (200). The existing `MAX_LOCK_HOLD_MS` slot stays — it's now wired (see D5).
...
- **H — Initial sync stall timeout.** Fixture FETCH hangs past `INITIAL_SYNC_BATCH_TIMEOUT_MS`; assert deadline fires, watermark not advanced, folder marked transient-fail, next cycle resumes from same watermark.
