...
- **Storage-aware read strategy:**
  ...
  - **Clean body** → STORED body text only (no IMAP), reusing the deterministic `cleanBody` heuristic. By default it removes recognized one-line or wrapped `On … wrote:` tails, final quote-only blocks, and `-- ` signatures. It keeps Outlook and `Original Message` blocks because they can contain forwarded evidence. Update: MCP message and thread reads pass the message subject, and an Outlook `From:` / `Sent:` or `Date:` / `Subject:` block (with an optional `Original Message` or underscore separator) ends the body only when that subject is a reply (`Re:`) and authored text precedes the block. Forwards and callers without a subject keep the block. No LLM, no new dependency.
