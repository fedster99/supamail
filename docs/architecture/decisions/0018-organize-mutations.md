...
- Flag changes are visible in the mirror immediately (write-through). A host can
  make a successful move converge immediately by forcing the known source and
  destination through one bounded supplemental live pass. Without that host
  follow-up, move and delete convergence remains eventual through the durable due
  rows and periodic exact reconciliation.
