...
SupaMail was extracted from a larger Signal system. The original context included CRM hydration, identity, belief, dashboard, Trigger.dev, MCP, and other product layers. The open-source SupaMail project needs a tighter boundary so the sync engine stays reusable and reliable.
...
SupaMail core does not own CRM, identity/belief modeling, calendar, contacts, sending, scheduling, AI workflows, MCP, or internal dashboard logic unless a future issue explicitly adds a separate package/process.
