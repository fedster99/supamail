...
| Reconcile provider deletes and missing-in-DB rows. | Implemented | Temp-table UID stream finds missing local rows and provider-only rows. When the live-window search is empty but mirrored live rows exist, an unfiltered UID stream confirms real deletion without backfilling provider-only archive UIDs; a second empty response still fails closed. Live DB tests verify provider tombstones, this empty-window edge, and `RECONCILE_BACKFILL`. |
