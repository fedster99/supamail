...
| Sync scheduling uses priority folders plus bounded round-robin. | Implemented | `PRIORITY_CUTOFF`, `MAX_PRIORITY_FOLDERS_PER_CYCLE`, `MAX_RR_FOLDERS_PER_CYCLE`, and `folder_rr_cursor` prevent lower-priority folder starvation. Sent reserves a bounded priority slot and also receives a lightweight 30-second metadata lane without multiplying reconcile/body/history work. |
