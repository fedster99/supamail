...
Flag scans and reconciles are also due-based and budgeted. Agents should not change this into "scan every folder every cycle" behavior; that is exactly the folder-explosion failure mode the old spec was trying to prevent. An opt-in LIST-STATUS wake probe may request fingerprints for the tracked path set in one strict command. It does not process those folders, bypass the sync caps, or replace exact reconciliation. An independent QRESYNC flag may replay changed flags and exact VANISHED UIDs from persisted UIDVALIDITY plus a deletion-complete mod-sequence cursor. That cursor is separate from flag-only CONDSTORE progress. QRESYNC is an optimization only: incomplete replay falls back, and the periodic exact UID audit remains authoritative.
