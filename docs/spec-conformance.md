...
| SMTP must not create false failures while the server is still processing DATA. | Implemented | SMTP connection and greeting setup keep the short `CONNECT_TIMEOUT_MS`. The final delivery response uses `SMTP_COMMAND_TIMEOUT_MS`, which defaults to the RFC 5321 value of 10 minutes. A local protocol test delays the final `250` beyond the IMAP command timeout and proves that the send still succeeds once. |
...
| Recover stale/orphaned account locks. | Implemented | Locks are never taken from a live session. Pooled sessions set server-side TCP liveness, so a vanished worker's locks are released by Postgres. Startup cleanup closes the orphaned `imap_sync_runs` rows only for accounts whose lock no session holds. Live DB tests cover a stale-heartbeat holder that stays untouched, a slow live sync, lost ownership stopping further writes, the applied TCP settings, and the SIGKILL/OOM path. |
| A sync run must not stay `running` after an in-process exception. | Implemented | `MirrorEngine.syncAccount` terminalizes an opened run before rethrowing unexpected account-lock, recovery, or finalization errors; focused regression coverage reproduces a Postgres pool-checkout failure immediately after run creation. Startup cleanup remains the crash/OOM backstop. |
...
| Sync scheduling uses priority folders plus bounded round-robin. | Implemented | `PRIORITY_CUTOFF`, `MAX_PRIORITY_FOLDERS_PER_CYCLE`, `MAX_RR_FOLDERS_PER_CYCLE`, and `folder_rr_cursor` prevent lower-priority folder starvation. A full safety pass gives newly tracked non-priority folders the existing round-robin slots before older due work, so discovery can synchronize them without increasing the folder budget. Inbox remains first in the bounded full-sweep set; Sent also receives a lightweight 30-second metadata lane without multiplying reconcile/body/history work. |
| Initial sync must be resumable and gap-safe. | Implemented | A frozen snapshot and newest-first history watermark make the initial pass resumable. Each cycle also mirrors one bounded live-head batch above the snapshot with an independent monotonic cursor, so new mail does not wait for the snapshot to finish. Spec-conformance proves both paths across three cycles. |
...
| Body fetches should be policy-controlled and lock-guarded. | Implemented | Body backlog/fetch uses the same account lock. A 32 KiB plain-text search extract and threading evidence commit before the pluggable `BodyStore` receives the full payload; database bodies remain the OSS default. `PATCH /accounts/:id/settings` can change `bodyFetchPolicy` to `immediate`, `lazy`, or `priority_then_backfill` for an existing account. In `BODY_STORAGE_MODE=parsed_only`, known-complete messages up to 4 MiB use a UID-set FETCH of at most 10 same-folder messages and 8 MiB of aggregate source. The bounded fetch and parse finish before body storage. Larger, unknown-size, and cap-limited messages use the streaming download path. The default `raw_mime` mode buffers only because raw persistence requires it. |
...
Folder discovery is due-based and persists provider seen/missing state. A host may force one discovery inside an authoritative full safety pass; the same successful LIST advances the normal discovery deadline, and supplemental live, Sent, and body-only lanes do not use the override. Full sync cycles process priority folders first, then a bounded number of non-priority folders using a stable round-robin cursor. The cursor indexes all eligible non-priority folders, not the changing subset currently due; selection skips not-due folders and advances past the last selected ordinary folder even if its later sync fails. Newly tracked non-priority folders take those existing round-robin slots before older due folders during the discovery pass; this does not add work beyond the configured folder budget or skip unselected ordinary work. Inbox remains first in that bounded priority set. Sent retains its normal priority 5 position and gets a supplemental due-based metadata refresh on the separate fast lane.

After round-robin selection and cursor advancement, ordinary selected folders execute in oldest reconcile-due order. Otherwise, recurring batches can let earlier positions repeatedly consume the smaller reconcile budget before later positions get a turn. Priority folders and preferred discovery work remain first. This ordering adds no folders or reconciliation slots and does not change retry or due intervals.

Between full sync cycles, a separate due-based Sent lane refreshes metadata at `SENT_SYNC_INTERVAL_MS` (30 seconds by default). It uses the same account and mailbox locks but deliberately skips discovery, flag scans, reconcile, body fetch, history, and full-account health/backoff transitions. Its durable sync run remains observable without advancing the account's full-sweep `last_sync_finished_at`; the next full cycle owns account health. The next full-sweep start is a hard deadline for supplemental Sent work: pending connection or throttle work and the active IMAP connection are interrupted, no more Sent accounts start, and the worker rechecks the full lane immediately. Any Sent-lane account-lock contention yields. These scheduler yields are neutral rather than false provider-failure events.

Flag scans and reconciles are also due-based and budgeted. Agents should not change this into "scan every folder every cycle" behavior; that is exactly the folder-explosion failure mode the old spec was trying to prevent. An opt-in NOTIFY layer may identify tracked dirty folders in real time. Rejection falls back to LIST-STATUS/STATUS; notification overflow disables the subscription, conservatively dirties Inbox and every tracked folder, drains the work through bounded catch-up batches, and then replaces the connection. An opt-in LIST-STATUS wake probe may request fingerprints for the tracked path set in one strict command. Neither signal processes folders, bypasses sync caps, or replaces exact reconciliation. An independent QRESYNC flag may replay changed flags and exact VANISHED UIDs from persisted UIDVALIDITY plus a deletion-complete mod-sequence cursor. That cursor is separate from flag-only CONDSTORE progress. QRESYNC is an optimization only: incomplete replay falls back, and the periodic exact UID audit remains authoritative.
