...
| Sync should emit durable observability events. | Implemented at mirror-event level | Sync runs, message/folder events, flag changes, reconcile backfills, and retention outcomes are stored or logged. Worker logs preserve sanitized error class/code/provider context, emit failed account outcomes at error severity, and emit partial outcomes at warning severity. Stable metrics/alert names remain an open operational layer. |
